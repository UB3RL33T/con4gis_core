        ...
        element
          ...
          .click(function()
          {
            ...
            var fnEnableWswgEditor = function(){
              if (wswgEditor && (typeof(wswgEditor) != 'undefined')) {
                if( $('#editor').length > 0 ){
                ...
                if( $('#ckeditor').length > 0 ){
                    if(typeof ckEditorItems == "undefined" || ckEditorItems == "" || ckEditorItems.length <= 1){
                        ckEditorItems = ['Cut', 'Copy', 'Paste', 'PasteText', 'PasteFromWord', '-', 'Undo', 'Redo', 'Bold', 'Italic', 'Underline', 'Strike', 'Subscript', 'Superscript', 'Blockquote', '-', 'RemoveFormat', 'NumberedList', 'BulletedList', 'Link', 'Unlink', 'Anchor', 'Image', 'FileUpload', 'Table', 'Smiley',  'TextColor', 'BGColor' ];
                    }
                    ...
                    try {
                      if (CKEDITOR.instances['ckeditor'] && typeof CKEDITOR.instances['ckeditor'] != "undefined") {
          ...
          $(content.dialogbuttons).each(function(index,value){
            ...
            dialogoptions.buttons.push( {cssClass: aClass,text: value.text,click:function() {
              if (value.type == 'send') {
                                ...
                                if(CKEDITOR.instances.ckeditor &&  ($('#ckeditor').length > 0)) {
