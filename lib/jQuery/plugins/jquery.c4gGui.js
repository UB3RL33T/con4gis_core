		...
		var fnHandleAjaxResponse = function(data, internalId) {
            	...
            	if (typeof(wswgEditor) != 'undefined') {
                    ...
                    if( $('#ckeditor').length > 0 ){
                        if(typeof ckEditorItems == "undefined" || ckEditorItems == "" || ckEditorItems.length <= 1){
                            ckEditorItems = ['Cut', 'Copy', 'Paste', 'PasteText', 'PasteFromWord', '-', 'Undo', 'Redo', 'Bold', 'Italic', 'Underline', 'Strike', 'Subscript', 'Superscript', '-', 'RemoveFormat', 'NumberedList', 'BulletedList', 'Link', 'Unlink', 'Anchor', 'Image', 'FileUpload', 'Table', 'Smiley',  'TextColor', 'BGColor' ];
                        }
                        ...
                        CKEDITOR.replace('ckeditor', {
                            ...
                            defaultLanguage:options.contaoLanguage,
			...
			if (typeof(content.dialogdata)!='undefined') {
				...
				if (tmpDialogDiv != null) {
                    ...
                    if(content.dialogid != "previewpost" && content.dialogid != "previewthread"){
					    fnEnableWswgEditor();
                    }else{
                        $('.BBCode-Area').each( function() {
                            $(this).html( wswgEditor.parseBBCode( $(this).html() ) );
